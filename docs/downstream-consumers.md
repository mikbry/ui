...
These are the sprints where consumer needs and mkui capabilities synchronize. **Revised 2026-05-21** after (a) Miky's app deadline (~3 weeks to usable mkui) surfaced via operator review, (b) StoneSketch's `crates/render` (2 854 lines of production wgpu) was confirmed available for extraction into `mkui-wgpu`, and (c) Codex review flagged that product-specific atoms (Miky's StatePill, RoleBadge, TierTag, row-builders, app-shell composites) belong in the consumer's downstream UI layer, not in mkui itself. The roadmap below ships **generic mkui primitives only**; the Miky-side mapping work that composes those primitives into Miky's catalog is tracked in [`mikbry/miky-internal#6`](https://github.com/mikbry/miky-internal/issues/6).

| Sprint | mkui delivers (generic primitives) | Consumer-side work unlocked |
|---|---|---|
...
| Sprint 2 (current, 4 days) | **Renderer foundations + first 2 generic atoms.** Batch 3: CI workflow + `mkui-core` clippy/std-trait fixes. Batch 4: port `stonesketch-render`'s HUD pipeline → `mkui-wgpu`; new `mkui-text` crate (cosmic-text + atlas behind `PlatformTextSystem` trait); shadcn-aligned `Badge` (canonical 4 variants) + `Dot` (status-color tokens + halo/animation modifiers). | Miky's app team begins integration smoke against a real wgpu surface with real text. `miky-internal#6` scaffolds `miky-ui` crate against the first two atoms. |
| Sprint 3 (~7 days) | **Window chrome + structural + remaining generic atoms/inputs.** `Window`, `Titlebar`, `Sidebar` (shadcn-aligned), `StatusBar`, `Popover`, `DropdownMenu`, `Tabs`, `ToggleGroup`, `Input`, `Avatar`, `Kbd`. All generic, all shadcn-aligned per [`docs/components/mkui-to-shadcn-mapping.md`](components/mkui-to-shadcn-mapping.md). | **Miky's app reaches "usable mkui" via the miky-ui adapter layer** — window chrome + generic atoms + Miky-specific atoms (StatePill, RoleBadge, TierTag, etc. composed in miky-ui from mkui generics) render end-to-end |
| Sprint 4 (~7 days) | **Remaining generic structural + deferred Sprint 2 hygiene.** `Card` (full slots), `Alert`, `Progress`, `ScrollArea` (already prototyped), `Separator`, `ResizeHandle`. Catch-up: mkui-c FFI safety (audit 1.1), MSRV declaration (audit 1.4), README rewrite (audit 1.5 / #7). | **Miky's catalog-complete via miky-ui** — full Operator Console v0 surface renders through Miky's composition layer on top of mkui generics. Miky-side compositions (PRRow, AgentCard, NeedsYouRail, AuditRow, SprintCard, ClientBanner, BatchHeader, GatesStrip, etc.) ship in `miky-ui`. |
| Sprint 5+ | StoneSketch adoption: `mkui-wgpu` adopted; `stonesketch-gui` retired or thin-re-exports. Phase 2.x audit cleanup (thiserror migration, `#[forbid(unsafe_code)]` rollout, native boundary decision). | StoneSketch finishes migration |
| Sprint 6+ | Platform-native text backends (`CoreTextSystem`, `DirectWriteSystem` behind the existing `PlatformTextSystem` trait) if consumer smoke surfaces pixel-mismatches that matter. Font/atlas pipeline polish. | App-store-grade text quality if needed |
...
- **mkui ships ~20 first-class generic components total** (5 atoms + 4 inputs + 11 structural — per [`docs/components/mkui-to-shadcn-mapping.md`](components/mkui-to-shadcn-mapping.md)). Product-specific atoms (StatePill, RoleBadge, TierTag, LedgerChip, NeedsYouRail, etc.), row-builders (PRRow, IssueRow, AgentCard, AuditRow, SprintCard, NeedsYouItem), and product-specific composites (ClientBanner, BatchHeader, GatesStrip) **do not ship in mkui** — they live in the consumer's UI crate (for Miky, that's `miky-ui` inside `mikbry/miky-internal`, scoped in issue #6 there).
- The "Miky reaches usable mkui at end of Sprint 3" claim assumes the miky-internal team ships the Miky-specific composition layer in parallel with mkui Sprint 3. Mkui's deliverables enable that work; the work itself happens in the miky repo.
